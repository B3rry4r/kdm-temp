...
const KickstartMyBizQuizPage: React.FC = () => {
  ...
  const submitQuiz = useCallback(async (isTimeout: boolean = false) => {
    ...
    try {
      await apiClient.post("/quiz/store/scores", {
        ...
        timeout: isTimeout,
        quiz_type: "kickstart-my-biz"
      });
      // Save user's answers and questions to localStorage
      localStorage.setItem('quiz-final-answers-kickstart-my-biz', JSON.stringify(answers));
      localStorage.setItem('quiz-final-questions-kickstart-my-biz', JSON.stringify(questions));
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-2xl mx-auto py-8">
      <div className="mb-4 flex justify-between items-center">
        <div className="font-bold text-lg">Kickstart My Biz Quiz</div>
...
export default KickstartMyBizQuizPage;
